import PathParser from '../../src/core/util/path-parser.js';
import expect from 'expect';

describe('Drone path parsing', function () {

    it('correctly calculates amount of photos taken from provided examples', function() {
        let parser = new PathParser('x^^x>>xvvx<<x');
        let route = parser.parse();
        expect(route.billboards.length).toBe(4);

        parser = new PathParser('x^xv');
        route = parser.parse();
        expect(route.billboards.length).toBe(2);
    });
